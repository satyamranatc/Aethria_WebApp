...
export default function ProjectCommandCenterPage({
  ...
  return (
    <div className="min-h-screen bg-[#F4F5F7] text-[#1D1D1F] flex flex-col font-[-apple-system,BlinkMacSystemFont,'SF_Pro_Display','Inter',sans-serif] selection:bg-[#4F46E5]/15 selection:text-[#4F46E5]">
      {/* Top Navigation Bar — Landing Aesthetic */}
      <header className="sticky top-0 z-30 h-14 bg-[#F4F5F7]/80 backdrop-blur-xl border-b border-black/[0.06] px-4 sm:px-8 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] uppercase tracking-[0.14em] font-semibold text-[#4F46E5] bg-[#4F46E5]/10 px-2 py-0.5 rounded-full border border-[#4F46E5]/15">
              VS Code Bridge
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          <div className="hidden sm:flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-500/10 text-emerald-700 text-xs font-semibold border border-emerald-500/20">
          ...
          <button
            ...
            className="px-4 py-1.5 rounded-full bg-[#1D1D1F] hover:bg-black text-white text-xs font-semibold shadow-xs hover:shadow-md transition-all flex items-center gap-1.5 cursor-pointer"
          >
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto p-4 sm:p-8 space-y-8">
        ...
        <div className="p-6 sm:p-10 rounded-[28px] bg-white border border-black/[0.08] shadow-[0_8px_30px_rgba(0,0,0,0.02)] space-y-6">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div className="space-y-2.5 max-w-2xl">
              <span className="inline-flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-[0.14em] text-[#4F46E5] bg-[#4F46E5]/10 px-3 py-1 rounded-full border border-[#4F46E5]/15">
                <Sparkles className="w-3 h-3" />
                Codebase Intelligence &middot; VS Code Bridge
              </span>
              <h1 className="text-3xl sm:text-4xl md:text-5xl font-semibold tracking-[-0.04em] text-[#1D1D1F] leading-[1.08]">
                Projects Command Center
              </h1>
            ...
            <div className="flex flex-wrap sm:flex-nowrap gap-2.5 shrink-0">
              <button
                ...
                className="px-4 py-2 rounded-full bg-[#F4F5F7] hover:bg-white text-[#1D1D1F] text-xs font-semibold border border-black/[0.06] transition-all flex items-center gap-2 cursor-pointer shadow-2xs"
              >
              ...
              <button
                onClick={() => setIsCreateModalOpen(true)}
                className="px-5 py-2 rounded-full bg-[#4F46E5] hover:bg-[#4338CA] text-white text-xs font-semibold shadow-[0_4px_16px_rgba(79,70,229,0.25)] transition-all flex items-center gap-2 cursor-pointer"
              >
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-6 border-t border-black/[0.05]">
            <div className="p-4 bg-[#F4F5F7]/70 rounded-2xl border border-black/[0.04]">
              <span className="text-[10px] uppercase tracking-[0.14em] font-semibold text-[#86868B] block">Active Projects</span>
              <div className="flex items-baseline gap-1 mt-1 font-mono">
                <span className="text-2xl font-bold text-[#1D1D1F]">{projects.length}</span>
                <span className="text-[10px] text-[#86868B]">tracked</span>
              </div>
            </div>

            <div className="p-4 bg-[#F4F5F7]/70 rounded-2xl border border-black/[0.04]">
              <span className="text-[10px] uppercase tracking-[0.14em] font-semibold text-[#86868B] block">Tracked Files</span>
              <div className="flex items-baseline gap-1 mt-1 font-mono">
                <span className="text-2xl font-bold text-[#4F46E5]">{totalFiles}</span>
                <span className="text-[10px] text-[#86868B]">in Cloud Brain</span>
              </div>
            </div>

            <div className="p-4 bg-[#F4F5F7]/70 rounded-2xl border border-black/[0.04]">
              <span className="text-[10px] uppercase tracking-[0.14em] font-semibold text-[#86868B] block">Sprint Deliverables</span>
              <div className="flex items-baseline gap-1 mt-1 font-mono">
                <span className="text-2xl font-bold text-[#1D1D1F]">{totalOpenTasks}</span>
                <span className="text-[10px] text-[#86868B]">milestones</span>
              </div>
            </div>

            <div className="p-4 bg-[#F4F5F7]/70 rounded-2xl border border-black/[0.04]">
              <span className="text-[10px] uppercase tracking-[0.14em] font-semibold text-[#86868B] block">Audit Health</span>
              <div className="flex items-baseline gap-1 mt-1 font-mono">
                <span className={`text-2xl font-bold ${totalOpenIssues > 0 ? 'text-[#D70015]' : 'text-emerald-600'}`}>
                  {totalOpenIssues}
                </span>
                <span className="text-[10px] text-[#86868B]">{totalOpenIssues > 0 ? 'needs review' : 'all clear'}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="relative flex-1 w-full max-w-md">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2.5 rounded-full bg-white border border-black/[0.06] text-xs text-[#1D1D1F] placeholder:text-[#86868B] focus:outline-none focus:border-[#4F46E5]/50 focus:ring-2 focus:ring-[#4F46E5]/10 shadow-xs"
            />
          </div>

          <div className="flex items-center gap-2 self-end sm:self-auto">
            <div className="flex items-center bg-white p-1 rounded-full border border-black/[0.06] text-xs shadow-2xs">
            ...
            <select
              ...
              className="px-3.5 py-2 rounded-full bg-white border border-black/[0.06] text-xs text-[#1D1D1F] shadow-2xs cursor-pointer focus:outline-none"
            >
        ...
        {isLoading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[1, 2, 3].map((n) => (
              <div
                key={n}
                className="h-64 rounded-[24px] bg-white border border-black/[0.06] p-6 animate-pulse space-y-4"
              >
                <div className="h-4 bg-black/[0.04] rounded-full w-1/3" />
                <div className="h-6 bg-black/[0.04] rounded-xl w-2/3" />
                <div className="h-16 bg-black/[0.04] rounded-xl" />
              </div>
            ))}
          </div>
        ) : filteredProjects.length === 0 ? (
          <div className="text-center py-20 p-8 rounded-[28px] bg-white border border-black/[0.08] shadow-sm space-y-4">
            ...
            <button
              onClick={() => setIsCreateModalOpen(true)}
              className="px-5 py-2 rounded-full bg-[#1D1D1F] hover:bg-black text-white text-xs font-semibold shadow-xs transition-all inline-flex items-center gap-1.5 cursor-pointer"
            >
              ...
              {filteredProjects.map((proj, idx) => {
                ...
                return (
                  <motion.div
                    ...
                    className="p-6 rounded-[24px] bg-white border border-black/[0.08] hover:border-[#4F46E5]/40 hover:shadow-[0_16px_36px_rgba(79,70,229,0.08)] transition-all flex flex-col justify-between space-y-4 cursor-pointer group"
                  >
                    <div className="space-y-3">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <span className="text-[10px] font-bold uppercase tracking-wider text-[#4F46E5] bg-[#4F46E5]/10 px-2.5 py-0.5 rounded-full border border-[#4F46E5]/15">
                            {proj.projectType}
                          </span>
                      ...
                      <div className="flex items-center gap-3.5 py-2 text-[11px] text-[#6E6E73] font-medium border-y border-black/[0.04]">
                        <span className="flex items-center gap-1">
                          <FolderCode className="w-3.5 h-3.5 text-[#4F46E5]" />
                          <strong className="text-[#1D1D1F] font-mono">{folderCount}</strong> folders
                        </span>
                        <span className="text-black/[0.15]">&middot;</span>
                        <span className="flex items-center gap-1">
                          <FileCode className="w-3.5 h-3.5 text-[#4F46E5]" />
                          <strong className="text-[#1D1D1F] font-mono">{fileCount}</strong> files
                        </span>
                        <span className="text-black/[0.15]">&middot;</span>
                        <span className="flex items-center gap-1">
                          <Code className="w-3.5 h-3.5 text-emerald-600" />
                          <strong className="text-[#1D1D1F] font-mono">{totalLoc.toLocaleString()}</strong> LOC
                        </span>
                      </div>

                      {/* Technologies & Languages */}
                      <div className="flex flex-wrap gap-1.5 pt-1">
                        {(proj.technologies || ['React', 'Node.js']).slice(0, 3).map((t, tIdx) => (
                          <span
                            key={tIdx}
                            className="px-2.5 py-0.5 rounded-full bg-[#F4F5F7] text-[#6E6E73] text-[10px] font-medium border border-black/[0.04]"
                          >
                            {t}
                          </span>
                        ))}
                        {(proj.technologies || []).length > 3 && (
                          <span className="px-2 py-0.5 rounded-full bg-[#F4F5F7] text-[#86868B] text-[10px] font-medium">
                            +{proj.technologies.length - 3}
                          </span>
                        )}
                      </div>
